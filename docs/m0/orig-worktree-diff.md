...
**M0-04c 结果（2026-09-07）**：用同一 `tools/build.sh release` 从脏树构建 candidate
（SHA-256 `dea1a66d0b57daf4…`），两例 COMPLETED，`1.flavia.res` 解析后与 reference 逐值精确相等
（`docs/m0/evidence/candidate/compare-*.json`）。因此下表 3 个 VERIFY 项全部改判 DEFER：
脏树修改对首条静力切片无可观测影响；`factw` 初始化仍应在 M1 作为防御性修复导入并单独证明。
...
| Fem.f90 | 10183–10197 | `block` 求 `sum_M_elem_diag`、`sum_K_diag` 写 unit 7（fort.7），调试用 | 所在分支需核对 | 结果无影响（04c） | DEFER（调试输出，不导入） |
...
| Output.f90 | 3882–3920 | tfix2026：`lcdofn(jdofn)` 改为 `lmdofn(jdofn)`，并加 `idofn>=1` 越界保护 | 输出分支；注释称“全自由度 deck 两者相同” | 是（输出路径），结果相同（04c） | DEFER（M8 单场分析时再评估） |
| Output.f90 | 3956–4030 | 无 `gpvar`/`gapg`/`ntstress` 的单元写 0 | 热/非接触单元 | 部分（非接触），结果相同（04c） | DEFER（M6.6 接触时再评估） |
| Output.f90 | 4417, 4604, 4953, 5031, 5111, 5281 | 热材料无 mechanical 段时 `material=' '` | `props(matno)%mechanical` 未关联 | 否 | DEFER（M8） |
| Stiff.f90 | 2422 | `factw=0.0_irk`，密度循环前初始化 | 组装体力/质量时 | **是**（NGRAV=1），结果相同（04c） | DEFER→M1-03：结果不依赖该值，但属防御性初始化，与 R17/R18 一并在 M1 导入并用 strict profile 证明 |
