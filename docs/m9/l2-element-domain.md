...
### 分歧已定位：`.nrt` 插值约束根本没被施加

不需要状态快照就定位到了——`1.chk` 的方程数直接把它写在脸上：
...
两个减法逐项对上。适配器路径施加了给定位移约束、**没有施加插值约束**，
于是多出 122 个本不该存在的方程——结构被放松了，位移偏大，
而且偏差随荷载线性增长，与实测完全吻合。

**这证伪了本轮的一个设计判断，必须记下来。** 我在 §6 把 `.nrt` 当作
「随网格原样带走、legacy 在两条路径上都读」的网格伴随文件，理由是
Global.f90:1489-1531 那一段没有 `yl_input_enabled` 守卫。
守卫确实没有——但整个 `global_data` 在适配器路径上就不执行，
所以「没有守卫」根本不蕴含「会被读到」。这是一个**从代码形状推断执行**的错误，
而本项目的纪律恰恰是「以执行为证据」。

因此 `.nrt` 插值表是一个**真正的迁移项**，不是直通项：
它需要进 ProblemState，由适配器读、由 commit 发布 `trans%nintf/listf/rintf`。
`mesh.format = "hstar-legacy-cor-ele-nrt"` 这个取值以及
`docs/m1/adapter-coverage.toml` 里那四条 `not_adapted` 行所引用的
「legacy 在两条路径上都读它」的证据，都要随之改写。这是下一步。

（顺带发现的第二条：`1bcs.flavia.*` 在 legacy 读取器路径上写出，在适配器路径上
是空的——`gid_bcs` 没走到。不在观察面上，单列登记。）
