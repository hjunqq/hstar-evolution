...
## 10. 材料域剩余能力的优先级建议（2026-09-15，实测为据）

### 10.1 先测一遍「有没有能用的 deck」——这一步改变了排序

§2.3 按代码列了 15 个本构模型。但**代码里有分支 ≠ 有可迁移的能力**：
迁移的完成判据是「真实 deck + 与冻结参考严格等价」，所以第一道筛子是
**这份 deck 的 legacy 路径本身能不能跑完**。逐个实测：

| 模型 / 能力 | 真实 deck | legacy 路径 | 结论 |
|---|---|---|---|
...
| `icreep`（徐变） | **无** | — | 全仓**没有**一个 SOLID 徐变 deck（`train_temp_creep` 是 `HEAT` 材料） |
| `kind_wt`（湿化） | **无** | — | 同上 |

**两个 SIGSEGV 需要单独定性**：是 legacy 原有行为，还是 M1 包装引入的缺陷？
现在不知道，**不要按「deck 坏了」记账**。这本身是一件小而明确的调查。

### 10.2 由此得出的结论：下一个瓶颈不是材料

仅存的两个「有可用 deck」的材料模型，**都要求 PARDISO**：

| deck | 求解器 | 其他差值 |
|---|---|---|
| `train05_slope_stability` | **PARDISO** | MCJOINT 准则（+4 字段）、4 材料 4 组、`type_nl=4`、`MAT_curve=2`（已支持） |
| `train06_concrete_damage` | **PARDISO** | **3-D**（`NDIMN=3`）、CONCRETE 多条记录 |

所以**材料域剩下的路被求解器挡着**，而不是被材料挡着。

### 10.3 优先级建议

| # | 能力 | 理由 | 工作量（以已验证链条的"第几次"计） |
|---|---|---|---|
| **1** | **PARDISO 求解器** | **唯一解锁项**：两个可用材料 deck 都要它。本身不是材料能力，但不做它材料域就停在这里 | 一条**新**链条：`.sol`/`.glb` 的求解器分支 + 运行存在面（PARDISO 的工作数组）。比一次材料能力**大**，且风险在「新求解器的数值可复现性」而非输入 |
| **2** | **MCJOINT** | 与 MC **同一模型同一分支**，只多 4 个字段（`ft/cft/sigmat/csigmat`）。骑在 #1 之上 | 材料能力的**第三次**，最轻的一次；另需 4 材料 4 组（多组已在 M6.4 打开） |
| **3** | **CONCRETE** | 有可用 deck，但同时带 **3-D** | 材料能力 + **3-D 这条独立主线**（`mini_3d` 是 M6.1 的规划算例） |
| **4** | **GOODMAN / DUNCANCHANG** | **先定性那两个 SIGSEGV**，再谈迁移。GOODMAN 还很可能是**新单元类型**而非纯材料 | 先是一次小调查，不是一次能力 |
| **5** | `icreep` / `kind_wt` / `jliqu` | **全仓没有可用 deck**。按 ADR-0008 §3：现阶段只要求 legacy 路径不被破坏 | 不做 |

**建议取 #1**，理由是它是唯一能让材料域继续走下去的一步；
若希望保持"每次一个材料能力"的节奏，则 #4 的**定性调查**是唯一不依赖 PARDISO 的选项，
且成本最小。

**不建议**现在做 #3（3-D 与材料两条主线会混在同一份验收证据里，
与 M6.5 那次"语义修正与新能力分开"的纪律相悖）。
