...
>
> **更正（2026-09-09，lead 自己的错误）**：后半句不由前半句推出，而且是错的。
> 盲区与链的层数无关，它关于**任何经由一个已被释放的全局才能到达的对象**。
>
> 实测两次：dev-fold2 删掉 `tcurves(i)%ttime_curve`（**一层**指针）的释放，979/979 全绿；
> lead 删掉 `prescrib(i)%leldofix`（**M3-03 时期就存在**的一层指针）的释放，
> release 979/979、MSan 979/979，两个都 rc=0。
>
> 所以这个盲区**先于整个折叠工作存在**，M3-03 的释放路径一直带着它而无人测过。
> 范围是 `commit_release` 里的**全部 20 处内层指针释放**：
> `element` 10（`ldofs`、6 个 `field%*`、3 个 `egaus%*`）、`group` 2、`listp_group` 2、
> `prescrib` 3、`tcurves` 2、`props` 1。**删掉其中任何一处，所有门禁都是绿的。**
>
> 这条错误的成因值得记下：lead 测了 `props` 一个案例，就把「它是唯一的两级链」
> 这个**成立的**事实，推广成「它是唯一处于盲区的对象」这个**不成立的**结论。
> 一次测量证明了盲区存在，没有证明盲区的边界在哪里。
...
**出口条件收紧（2026-09-09）**：以 `props%mechanical%solid` 单独作阳性对照**不充分**——
它只证明 valgrind 能工作，不证明其余 19 处干净。出口条件改为：**逐个记录数组各取一处内层指针，
删除其释放，valgrind 必须报出该次泄漏**；六个记录数组（`element`/`group`/`listp_group`/
`prescrib`/`tcurves`/`props`）各至少一次。任何一个数组没有被证明可检出，
该数组的内层释放就仍处于未验证状态，须如实记为 NOT PERFORMED。
