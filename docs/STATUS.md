...
| M6.5 强度折减 + 材料曲线（MAT_DE / mat_curve） | 完成 4/4 | **待签收** | `docs/m6/material-domain.md` §9 + `cases/golden/plasticity/slope_srm`（600 块 541 200 值 `max|d|=0`，**非零 PLASTICSTRAIN 18 924/45 100**） |
...
**复核已完成（2026-09-13），范围如裁定，没有扩大**：新增 `tools/yl_coverage_check.py`
（六条断言 + 9 例自检）与登记表 `docs/m1/adapter-coverage.toml`，接入 `tools/build.sh adapter`。
**没有编制 M1 验收矩阵，也没有重做 M1 的任何既有结论。**
现在「适配器覆盖了哪些读取站点」是可证实也可证伪的：`154 = 133 + 21`。
M1 的形式验收缺口（没有阶段矩阵、条件解除未经第二方逐条核）**依然存在，本次复核不声称补上了它**。
...
- **R29 已关闭（2026-09-13）**：`tools/yl_coverage_check.py` 接入 `tools/build.sh adapter`。
  **on-path 154 = 已标记 133 + 登记未适配 21**，登记表每次运行按名字打印。
  关闭当场证实了 R29 自己举的例子：`Load.f90:231` 一直是覆盖的，只是标记与诊断 id 写错了名字。
- **R30 未闭合**：「出处账本的 runtime 来源是散文、无机械对账」，使某类断言不成立，已登记关闭条件。
