...
|---|---|---|---|---|---|---|
| **静力（2D Q4 线弹性 / 重力 / 单增量 / PROFILE）** | 已登记 157 处 | 已摸清（有具名缺口） | `ProblemState` + `deck_residue_t` | **仅 bootstrap** | **状态等价已证；数值等价未建立** | M1～M4 |
...
- **回归证据**：状态等价成立（影子差分 `MATCH=324 MISMATCH=0`，仅 `model_ready`；
  红线 `MATCH=64 MISMATCH=0`）；**数值等价未建立**，属 M4-02，且其容差尚未定案（R28）。
