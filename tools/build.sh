...
# side, M1-02) first, then the legacy module order below.
#
...
for f in "${DIAG_SRCS[@]}"; do [ -f "$ROOT/$f" ] || { echo "build.sh: missing source $ROOT/$f" >&2; exit 4; }; done
...
# path>|<absolute path>": repository files (DIAG_SRCS) by their repo-relative
# path, legacy files by their name inside --src.
SRC_ENTRIES=()
